...
type StateChangeListener = (state: VideoStreamPlayState, previousState: VideoStreamPlayState) => void;
...
const createPlayer = (sourceType: VideoStreamSourceType): Player =>
  match(sourceType)
...
const detectSourceType = (url: string): VideoStreamSourceType =>
  match(url.toLowerCase())
    .when(
      (url) => url.endsWith(".mpd") || url.includes("dash"),
      () => "dash" as VideoStreamSourceType,
    )
    .when(
      (url) => url.endsWith(".m3u8") || url.includes("hls"),
      () => "hls" as VideoStreamSourceType,
    )
    .otherwise(() => "video" as VideoStreamSourceType);

export const initializePlayer = (sourceType: VideoStreamSourceType): RIO.ReaderIO<VideoStreamEnv, void> =>
  pipe(
...
export const loadSource = (source: VideoStreamSource): RIO.ReaderIO<VideoStreamEnv, void> =>
  pipe(
...
export class ObjectVideoStream {
  ...
  get currentSourceType(): VideoStreamSourceType {
  ...
  loadSource = (source: VideoStreamSource): IO.IO<void> => loadSource(source)(this._videoStreamEnv);
