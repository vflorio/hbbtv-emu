...
import type { MediaSource, PlayerError, PlayerEvent, PlayerEventListener, PlayerEventType } from "../types";
...
export type PlayerEnv = {
  ...
  setState: (state: StreamPlayState) => IO.IO<void>;
  setSource: (source: MediaSource | null) => IO.IO<void>;
...
export const setState = (newState: StreamPlayState, loggerName: string): RIO.ReaderIO<PlayerEnv, void> =>
  pipe(
    RIO.ask<PlayerEnv>(),
    RIO.flatMap((env) =>
...
export const loadSourceBase = (source: MediaSource, loggerName: string): RIO.ReaderIO<PlayerEnv, void> =>
  pipe(
    RIO.ask<PlayerEnv>(),
    ...
    RIO.flatMap(() => setState(StreamPlayState.CONNECTING, loggerName)),
  );
...
export const stop = (loggerName: string): RIO.ReaderIO<PlayerEnv, void> =>
  pipe(
    ...
    RIO.flatMap(() => setState(StreamPlayState.STOPPED, loggerName)),
  );
...
export const releaseBase = (loggerName: string): RIO.ReaderIO<PlayerEnv, void> =>
  pipe(
    ...
    RIO.flatMap(() => setState(StreamPlayState.IDLE, loggerName)),
  );
