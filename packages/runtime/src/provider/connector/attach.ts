...
export const applyStrategy = <T>(strategy: AttachStrategy, detected: DetectedElement, instance: T): IO.IO<void> =>
  match(strategy)
    .with("non-visual", () => nonVisualObjectStrategy(detected, instance as NonVisualOipfObject))
    .with("visual", () => visualObjectStrategy(detected, instance as VisualOipfObject))
    .exhaustive();
